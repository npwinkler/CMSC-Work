...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname graded-lab2) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f ())))
;Lab2 - Nolan Winkler
...
(build-list 7 (lambda (n) (time (recursive-tiles2 
                                 (expt 2 (+ n 2))))))
(build-list 3 (lambda (n) (scale 10 (recursive-tiles2 
                                     (expt 2 (+ n 2))))))
(build-list 8 (lambda (n) (= 
                           (expt 2 (+ n 2)) 
                           (image-width (recursive-tiles2 
                                         (expt 2 (+ n 2)))))))
(build-list 8 (lambda (n) (= 
                           (expt 2 (+ n 2)) 
                           (image-height (recursive-tiles2 
                                          (expt 2 (+ n 2)))))))

;; GRADER'S NOTE: I decided ultimately to grade the function recursive-tiles2
;; since it was the closest to being correct. In the future, you will no doubt
;; be graded for either the first function you list or the one that matches the
;; assignment, and possibly lose points for all the excess coding.

;; GRADER'S NOTE: Locals are very useful to speed up your code or when your
;; function just requires a very short wrapper around a different subroutine.
;; However, the huge number of locally defined functions in your code makes it
;; somewhat unwieldy. Consider in the future defining these locally defined
;; functions as helper functions instead. This will allow you to more
;; rigorously test them (and make your code better organized and easier to 
;; read).

;; GRADER'S NOTE: Please use correct indentation and cap lines at 80 characters.
;; Note that although I did not take points off for this, comments have their
;; own correct indentation. Remember Ctrl+A followed by the tab key will fix
;; indentation issues in DrRacket.

;; GRADER'S NOTE: Always write tests for your code. Every function you write
;; needs to have a few tests associated with it.

;; recursive-tiles 
;; - correctness             : 20/30
;; - subroutine organization : 10/30
;; - uses local well         : 8/13
